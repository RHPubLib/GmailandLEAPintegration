-- patron_emails_full.sql
-- Returns one row per unique email address for full-access cardholders.
...
--   1, 3, 6, 8, 14, 24, 25, 26, 29 = Full-access cardholder types at RHPL
--
-- Output columns: EmailAddress, PatronID
-- Used by: SSRS subscription → CSV email → patron_sync.py → Google Sheet "Full" tab
...
WITH RankedPatrons AS (
    ...
    WHERE PR.EmailAddress IS NOT NULL
      ...
      AND P.PatronCodeID IN (1, 3, 6, 8, 14, 24, 25, 26, 29)
